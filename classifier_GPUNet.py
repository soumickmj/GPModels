...
class Classifier(LightningModule):
    def __init__(self, **kwargs):
        ...
        if kwargs['network'] == "resnet18":
            ...
            # self.net = GP_UNet(n_classes=len(self.classIDs)) # use only when resuming old checkpoints doesn't have upalgo in name 
            if self.hparams.Relu == "leaky":
                ...
                self.net = GP_UNet(in_channels= (4 if self.hparams.contrast == "allCont" else 1), n_classes=len(self.classIDs),up_mode=self.hparams.upalgo, dropout=self.hparams.dropout, out_act=self.hparams.out_act)
            #self.net = GP_UNet(n_classes=len(self.classIDs), depth=self.hparams.depth, wf=self.hparams.wf, up_mode=self.hparams.upalgo)
    ...
    def prepare_dataset_splits(self):
        ...
        if self.hparams.Dataset == "Brats20" and self.hparams.amount == "full":
            X, m, y = load_Brats_full(
                self.hparams.amount,
                self.hparams.orient,
                f"{self.hparams.main_path}/Brats2020/Dataset",
                self.hparams.contrast,
            )
            ...
            if "Initial" in self.hparams.trainID or self.hparams.Dataset == "China":
                ...
                data_seg = pickle.load(
                    open(
                        f'{self.hparams.main_path}/Brats2020/Dataset/pickles/{self.hparams.amount}/{self.hparams.orient}/{self.hparams.contrast}_training_data.pickle',
                        'rb',
                    )
                )
                ...
                y = pickle.load(
                    open(
                        f'{self.hparams.main_path}/Brats2020/Dataset/pickles/{self.hparams.amount}/{self.hparams.orient}/{self.hparams.contrast}_labels.pickle',
                        'rb',
                    )
                )
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(
            TumourDataset(
                ...
                self.m_train,
                self.y_train,
                maxinorm=self.hparams.normmode == 3,
                transforms=transforms.Compose(
                    self.transform2 if self.hparams.MRIAug else self.transform1
                ),
...
def main():
    ...
    run_prefix=f"GPUNet_150e_{out_act}"    # _wnbact_deter  _2  _bilinear _sinc
    ...
    network: str = "GP_UNet"  #"GP_UNet" #"resnet18" # "GP_reconresnt"
